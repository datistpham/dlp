                    ...
                    fetch("".concat(window.livewire_app_url, "/livewire/message/").concat(payload.fingerprint.name), {
                        ...
                        if (response.ok) response.text().then((function(response) {
                            ...
                            if (419 === response.status) {
                                ...
                                store.sessionHasExpired = !0, _this.showExpiredMessage()
        ...
        return setInterval((function() {
            if (!1 !== node.isConnected) {
                ...
                if (!directives.missing("poll")) {
                    ...
                    store.livewireIsInBackground && !directive.modifiers.includes("keep-alive") && Math.random() < .95 || directive.modifiers.includes("visible") && !inViewport(directive.el) || store.livewireIsOffline || component.addAction(new _default$2(method, directive.params, node))
                }
            }
        }), interval)
    }
                ...
                return this.dollarWireProxy = new Proxy({}, {
                    get: function(object, property) {
                        ...
                        if ("string" == typeof property && property.match(/^emit.*/)) return function() {
                            ...
                            return "emitSelf" === property ? store.emitSelf.apply(store, [component.id].concat(args)) : "emitUp" === property ? store.emitUp.apply(store, [component.el].concat(args)) : store[property].apply(store, args)
                        };
